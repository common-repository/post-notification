	...
	while (false !== ($file = readdir ($dir_handle))) {
		if(is_dir(POST_NOTIFICATION_PATH . $file) && $file[0] != '.' && $file[0] != '_') {
			if(post_notification_is_profile(POST_NOTIFICATION_PATH . $file)){
				$en_profiles .= "<option value=\"$file\" ";
				if($file == $profile) $en_profiles .= ' selected="selected"';
				$en_profiles .= ">$file</option>";
			}
		}
	}
	closedir($dir_handle); 
	...
	<tr class="alternate">
		...
		<td>
			<?php 
				...
				foreach($wp_filter['the_content']  as $filter_level ){
					foreach($filter_level as $filter){
						echo '<input type="checkbox"  name="the_content[]" value="' .  $filter["function"] . '" ';
						if(in_array($filter["function"], $rem_filters)) echo ' checked="checked" ';
						
						echo '>' .  $filter["function"] . '</input><br />';
					}
				}
			?>
		</td>	
...
<table width="100%">
	...
	<tr class="alternate">
		...
		<td> 
			...
			<?php _e('Your profile must support Captchas.', 'post_notification'); ?>
		</td>
	</tr>
