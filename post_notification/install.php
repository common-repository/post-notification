...
function post_notification_install(){
	...
	maybe_create_table($t_emails, $sql);
	
	$sql = "ALTER TABLE $t_emails ADD subscribe_ip INT UNSIGNED NOT NULL default 0";
	maybe_add_column($t_emails , 'subscribe_ip', $sql);
	
	$sql = "ALTER TABLE $t_emails ADD format tinyint NOT NULL default 0";
	maybe_add_column($t_emails, 'format', $sql);

	$sql = "ALTER TABLE $t_emails ADD send_type tinyint NOT NULL default 0";
	maybe_add_column($t_emails, 'send_type', $sql);
	
	$sql = "ALTER TABLE $t_emails ADD send_datum int NOT NULL default 0";
	maybe_add_column($t_emails, 'send_datum', $sql);
	...
	$sql = "CREATE TABLE $t_posts (
				...
				PRIMARY KEY  (post_ID)
		   )"; /// \todo Add alter Table to downsize to tinyint
	...
	//*****************************************************//
	//**  Create WPPRAEFIX_post_notification_queue table **// 
	//*****************************************************//
	...
	$sql = "CREATE TABLE $t_queue (
				...
				INDEX (state, email_id),
				INDEX (idx)
		   )"; 
	
	maybe_create_table($t_queue, $sql);
	
	//****************************************************//
	//**  Create WPPRAEFIX_post_notification_subs table **// 
	//****************************************************//
	
		
	$sql = "CREATE TABLE $t_subs (
					email_id int( 11 ) NOT NULL,
					obj_id bigint(20) NOT NULL,
					obj_type  int NOT NULL,
					UNIQUE(email_id, obj_id, obj_type),
					INDEX(obj_type,obj_id)				
			   )"; 
	maybe_create_table($t_subs, $sql);
	...
	add_option('post_notification_nextmove', time(), 'When to send the next mail', 'yes');
	add_option('post_notification_emptyqueue', 1, 'Is the Queue empty?', 'yes');
